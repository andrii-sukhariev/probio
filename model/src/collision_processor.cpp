...
{
    sf::Vector2f tl = game_object->m_object_box.tl_position + sf::Vector2f(dx, 0.f);
...
{
    sf::Vector2f tl = game_object->m_object_box.tl_position + sf::Vector2f(0.f, dy);
    sf::Vector2f sz = game_object->m_object_box.size;
    ...
    for (int x = tl.x / tile_width; x < (tl.x + sz.x) / tile_width; ++x)
    {
        ...
        if (terrain.isInside(x, y) && terrain.at(x, y) == 1 && game_object->m_velocity.y <= 0)
        {
            game_object->m_collision_info.ceiling_collision = true;
        }
    }
}